...
//    window.location.href = '/pages/project-one.html'; // Navigate to b.html after setting the data in Local Storage
//  }
// Alle Links mit data-target sammeln
document.querySelectorAll('.project-link').forEach(link => {
  link.addEventListener('click', (e) => {
    e.preventDefault();

    const targetId = link.dataset.target; // z.B. "div1"
    const dataToSend = { id: targetId, className: 'active' };

    localStorage.setItem('data', JSON.stringify(dataToSend));

    // zur Projektseite weiterleiten
    window.location.href = '/pages/projectone.html';
  });
});
